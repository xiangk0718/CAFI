...
from ._utils import LAVT, LAVTOne, LAVTCatt
from .counterfactual_backbone import CFMultiModalSwinTransformer

__all__ = ['lavt', 'lavt_one', 'lavt_catt']
...
###############################################
# LAVT Catt: apply causal attention at LAVT   #
###############################################

def _segm_lavt_catt(pretrained, args):
    # initialize the SwinTransformer backbone with the specified version
    if args.swin_type == 'tiny':
        embed_dim = 96
        depths = [2, 2, 6, 2]
    ...
    out_indices = (0, 1, 2, 3)
    backbone = CFMultiModalSwinTransformer(embed_dim=embed_dim, depths=depths, num_heads=num_heads,
                                         window_size=window_size,
                                         ape=False, drop_path_rate=0.3, patch_norm=True,
                                         out_indices=out_indices,
                                         use_checkpoint=False, num_heads_fusion=mha,
    ...
    if pretrained:
        print('Initializing Conuterfactual Multi-modal Swin Transformer weights from ' + pretrained)
        backbone.init_weights(pretrained=pretrained)
    else:
        print('Randomly initialize Multi-modal Swin Transformer weights.')
        backbone.init_weights()
    model_map = [SimpleDecoding, LAVTCatt]
...
def _load_model_lavt_catt(pretrained, args):
    model = _segm_lavt_catt(pretrained, args)
    return model

def lavt_catt(pretrained='', args=None):
    return _load_model_lavt_catt(pretrained, args)