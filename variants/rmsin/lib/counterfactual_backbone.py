...
class CFMultiModalSwinTransformer(nn.Module):
    def __init__(self,
    ...
    def forward(self, x, l, l_mask, nl, nl_mask):
        """Forward function."""
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww, l, l_mask, nl, nl_mask)
    ...
    def train(self, mode=True):
        ...
        super(CFMultiModalSwinTransformer, self).train(mode)
        self._freeze_stages()
...
class MMBasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, l, l_mask, nl, nl_mask):
        """ Forward function.
        ...
        # PWAM fusion
        x_residual = self.fusion(x, l, l_mask, nl, nl_mask)
...
class PWAM(nn.Module):
    ...
    def forward(self, x, l, l_mask, nl, nl_mask):
        ...
        lang, _, _ = self.image_lang_att(x, l, l_mask, nl, nl_mask)  # (B, H*W, dim)
...
class SpatialImageLanguageAttention(nn.Module):
    def __init__(self, v_in_channels, l_in_channels, key_channels, value_channels, out_channels=None, num_heads=1):
        ...
        self.causalGate = CausalFeatureGate(self.value_channels, self.value_channels)
    ...
    def forward(self, x, l, l_mask, nl, nl_mask):
        ...
        # negative attention map
        nega_key = self.f_key(nl)
        nega_value = self.f_value(nl)
        nega_key = nega_key * nl_mask
        nega_value = nega_value * nl_mask
        nega_key = nega_key.reshape(B, self.num_heads, self.key_channels//self.num_heads, n_l)
        nega_value = nega_value.reshape(B, self.num_heads, self.value_channels//self.num_heads, n_l)
        nl_mask = nl_mask.unsqueeze(1)

        nega_sim_map = torch.matmul(query, nega_key)
        nega_sim_map = (self.key_channels ** -.5) * nega_sim_map
        nega_sim_map = nega_sim_map + (1e4*nl_mask - 1e4)
        nega_sim_map = F.softmax(nega_sim_map, dim=-1)

        # output feature
        positive_att = torch.matmul(sim_map, value.permute(0, 1, 3, 2))  # (B, num_heads, H*W, self.value_channels//num_heads)
        negative_att = torch.matmul(nega_sim_map, nega_value.permute(0, 1, 3, 2))
        
        base = positive_att 
        diff = positive_att - negative_att
        base = base.permute(0, 2, 1, 3).contiguous().reshape(B, HW, self.value_channels)  # (B, H*W, value_channels)
        base = base.permute(0, 2, 1)  # (B, value_channels, HW)
        diff = diff.permute(0, 2, 1, 3).contiguous().reshape(B, HW, self.value_channels)  # (B, H*W, value_channels)
        diff = diff.permute(0, 2, 1)  # (B, value_channels, HW)
        gate = self.causalGate(base)
        out = base + gate  * diff
        #out = diff
        ...
        return out, sim_map, nega_sim_map

class CausalFeatureGate(nn.Module):
    def __init__(self, c_in, c_hidden):
        super().__init__()
        self.weight = nn.Sequential(
            nn.Conv1d(c_in, c_hidden, 1, 1),
            nn.ReLU(),
            nn.Conv1d(c_hidden, c_in, 1, 1),
            nn.Tanh()
        )
